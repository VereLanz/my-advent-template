...
if __name__ == "__main__":
    if len(sys.args) < 2 or not isinstance(sys.args, int):
        raise TypeError("The first command line argument has to be an integer representing the day of December for AoC.")
    # must be between 1 and 25
    day_nr = int(sys.args[1])
    here = Path(__file__).parent
    
    # copy day_template into f"day{day_nr}".py
    shutil.copy(Path(here, "my_advent", "day_template.py"), Path(here, "my_advent", f"day{day_nr}.py"))
